...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        ...
        case 37: //车辆违章查询
        {
            [self queryCarViolationInfoCall];
        }
            break;
        default:
            break;
...
{
    [self waitLoading:YES];
    [self sendRequest:[MOBAEnvironmentRequest environmentRequestByCity:@"南京" province:@"江苏"]];
}
...
- (void)bankCardQueryCall
{
    [self waitLoading:YES];
    //6228482898203884775
    [self sendRequest:[MOBABankCardRequest bankCardRequestWithCard:@"6217001690000947722"]];
}
...
{
    [self waitLoading:YES];
    [self sendRequest:[MOBAUserCenterRequest userRigisterRequestByUsername:self.userName password:@"123456" email:self.userEmail]];
}
...
- (void)userLoginRequest
{
    ...
    [MobAPI sendRequest:[MOBAUserCenterRequest userLoginRequestByUsername:self.userName password:@"123456"]
               onResult:^(MOBAResponse *response) {
                   [self resultWithResponse:response];
...
{
    [self waitLoading:YES];
    [self sendRequest:[MOBAUserCenterRequest userPasswordChangeRequestByUsername:self.userName oldPassword:@"123456" newPassword:@"654321" mode:@"1"]];
}
...
//    NSString * item  = @"subName" ;
//    NSString * itemBase = [MOBFData stringByBase64EncodeData:[MOBFString dataByBase64DecodeString:item]];
//    NSString * name = @"vhbvb" ;
//    NSString * nameBase = [MOBFData stringByBase64EncodeData:[MOBFString dataByBase64DecodeString:name]];
...
{
    [self waitLoading:YES];
    [self sendRequest:[MOBAUserCenterRequest userPasswordRetrieveRequestByUsername:self.userName]];
}
...
#pragma mark 车辆违章信息相关查询
- (void)queryCarViolationInfoCall
{
    [self waitLoading:YES];
    [self sendRequest:[MOBACarViolationRequest queryCarViolationInfoByPlateNumber:@"苏FT218Y"
                                                                        plateType:@"02"
                                                                        engineNumber:@"M92654"
                                                                         areaCode:@"ntg"]];
}
